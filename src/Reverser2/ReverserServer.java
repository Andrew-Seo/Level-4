...
public class ReverserServer extends Thread {
	...
	// Socket and int for your client number
	ReverserServer(Socket sock, int x) {
	...
	public void run() {
		try {
			...
			// it's a getter)
			Scanner kb = new Scanner(socket.getInputStream());
			...
			while (true) {
				try {
					...
					// 6. read each line using your BufferedReader. This is the
					// user
					// input
					String input = kb.nextLine();
					System.out.println(input);
					// 7. break out of the loop if the user enters a certain
					// word or
					// it returns null.
	...
	public static void main(String[] args) throws IOException {
		...
		// 11. Create a ServerSocket on port 9898
		ServerSocket sockserv;
		try {
			sockserv = new ServerSocket(9898);

			try {
				while (true) {
					// 12. call the accept method on your ServerSocket and store
					// it
					// in a Socket Variable
					// Socket sock;

					// 14. create an instance of a ReverserServer and pass it
					// your
					// listener
					// and client number.
					ReverserServer s;
					try {
						s = new ReverserServer(sockserv.accept(), tempClientNum++);
